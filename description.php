...
if (isset ( $_GET ['action'] )) {
	...
	if ($result == 'save') {
		...
		if (isset ( $_GET ['id'] )) {
			...
			$query = "delete from materialdescription where id = '" . $id . "';";
...
<div class="container">
			...
			<a href="description/create.php" class="btn btn-warning">Hinzuf&uuml;gen</a>
		</div>
		<div class="row container">
			<table class="table table-striped">
				<thead>
					<tr>
						<th class="col-md-11">Materialbeschreibung</th>
						<th class="col-md-1">Aktion</th>
					</tr>
				</thead>
				<tbody>
					<?php
					$query = "SELECT id, description FROM materialdescription";
					$result = $conn->query ( $query );
					
					while ( $res = $result->fetch_assoc () ) {
						echo '<tr>';
						echo '<td>' . $res ['description'] . '</td>';
						echo '<td class="text-right"><a href="description/edit.php?id=' . $res ['id'] . '"><i class="fa fa-pencil"></i></a> <a href="' . $_SERVER ["PHP_SELF"] . '?action=delete&id=' . $res ['id'] . '"><i class="fa fa-trash"></i></a></td>';
						echo '</tr>';
					}
					?>
				</tbody>
